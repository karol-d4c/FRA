---
title: "**Single Cell Response Curve (SCRC) - R package**"
...
abstract:
  # The package SCRC is designed to estimate channel capacity between finite state input and multidimensional continuous output from experimental data. For efficient computations, it uses an iterative algorithm based on logistic regression. In addition, functions to estimate mutual information and calculate probabilities of correct discrimination between a pair of input values are implemented. The method is published in PLOS Computational Biology [@jetka2019].
...
output:
  pdf_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{SCRC User Manual}
...
The package can be directly installed from [GitHub](https://github.com/sysbiosig/SCRC).  For installation, open RStudio (or base R) and run following commands in the R console

```c
install.packages("devtools") # run if 'devtools' is not installed
library(devtools)
install_github("sysbiosig/SCRC")
```
...
> [https://github.com/sysbiosig/SCRC/issues](https://github.com/sysbiosig/SCRC/issues)
...
* Define SCRC and confusion matrix

# Package structure

The SCRC package provides their functionalities with three main functions:

1. `SCRC()`- estimates the single cell dose-response curve and  cell-to-cell heterogeneity structure. 

1. `plotConfusionMatrix()` - visualises the cell-to-cell heterogeneity structure as fractions of cells stimulated with one dose (rows) with responses typical to either of the doses (columns).

2. `plotSCRCWaves()` visualises single cell dose-response curve, line correspons do the fractions of cells that exhibit different response levels as the dose increases, whereas the bands representing the cell-to-cell heterogeneity structure.
...
The function `SCRC()` takes `data` in the form of the object `data.frame` with a specifc structure of rows and columns. 
...
## Data example 
An example of the input `data.frame`, which contains the multivariate dose-responses to IFN-a2a in **monocytes CD14+ CD16-** presented in the **MP** is available within the package under the variable `SCRC::data.scrc.cytof`.  It has the following format
...
knitr::kable(head(SCRC::data.itrc.cytof))
```
...
```c
head(SCRC::data.scrc.cytof)
```

# Esitmation of the SCRC
...
The result of the function is an object of class `SCRCModel`, that contains results of the estimator. To see the results call :
```c
print(model)
```
To get the SCRC (cumulative frequency) and cell-to-cell heterogeneity (confusion matrix) please call `model$scrc` and `model$confusion.matrix`, respectively.

The result can be visualised using one of our plots, as it was presented in the publication.

SCRC can be plotted using function:
...
```c
plotConfusingMatrix(model)
```

## Example of usage 
Below, we present an application of `SCRC` package to the case of the multivariate dose-responses to IFN-a2a in **monocytes CD14+ CD16-** described above. 
```c
library(SCRC)
model <-
  SCRC(
    data = SCRC::data.itrc.cytof,
    signal = "Stim",
...
SCRC::plotSCRCWaves(model = model) 
```


```{r scrc_cytof_4, include=TRUE, eval=TRUE, cache=TRUE }
SCRC::plotCofusionMatrix(model = model)
```
...
## SCRC
Calculation of the SCRC and cell-to-cell heterogeneity with default settings is perfomed by the command
...
* model - SCRCModel object return by SCRC function
* title_ - character, specify title of plot, default "Information Theoretic Response Curve"
* xlab_	- character, label of x axes, default "States number"
* ylab_	 - character, label of y axes and legend title, default "Signal levels"
* fill.guide_	- logical, specify if legend should be displayed
* ylimits_	 - logical (TRUE  or FALSE) or vector of minimum and maximum of y axes,
